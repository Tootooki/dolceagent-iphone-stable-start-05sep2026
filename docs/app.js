...
Promise.all([fetch('sheet-data.json').then(r=>r.json()),fetch('sheet-layout.json').then(r=>r.json())]).then(([sheet,layout])=>{
  ...
  window.dolceMenu=Object.freeze({openPanel:(panel,fromSettings=false)=>{if(['settings','workspace','products','periods','column-order'].includes(panel))openMenu(panel,null,fromSettings)},close:()=>closeMenu()});
