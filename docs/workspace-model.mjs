...
import {ACTION_BRAND, ACTION_NOTICE, ACTION_STATUSES, createActionStatuses, mockActionsFor} from './actions-model.mjs?v=70';
...
export const CONFIG = {
  ppc: {fields: [field('view', 'View', ['Campaigns', 'Targets']), field('status', 'Status', ['All', 'Active', 'Paused'])]},
...
export function createWorkspaceState() {
  return {active: 'accounting', tab: 'accounting', density: 'compact', actionStatuses:createActionStatuses(), options: Object.fromEntries(
...
export function setOption(state, id, key, value) {
  ...
  state.options[id][key] = value; return true;
}
export function viewFor(state, id = state.active) {
  ...
  if(id==='actions'){
    ...
    return {id,title:`${ACTION_BRAND} · ACTIONS`,headers:['ID','TEAM','TYPE','TODO','STATUS','ACTION'],
      rows:actions.map(action=>[action.id,action.area,action.kind,action.todo,action.status,'A']),actions,
      summary:`${actions.length} mock examples · ${options.area} · ${options.status}`,notice:ACTION_NOTICE};
  }
