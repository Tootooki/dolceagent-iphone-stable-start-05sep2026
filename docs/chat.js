...
function updateViewport(){
  if(panel.hidden)return;const viewport=window.visualViewport;
  const bottom=viewport?viewport.offsetTop+viewport.height:window.innerHeight;
  const inset=Math.max(0,shell.getBoundingClientRect().bottom-bottom);
  widget.style.setProperty('--chat-keyboard-inset',inset+'px');
  widget.dataset.short=String((viewport?.height||window.innerHeight)<420);
  const typing=panel.contains(document.activeElement)&&['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName);
  widget.dataset.keyboard=String(inset>100||Boolean(viewport&&window.innerHeight-viewport.height>100&&typing));
}
function stopVoice(){
...
function closeChat({restoreFocus=true}={}){
  const wasOpen=!panel.hidden;stopVoice();panel.hidden=true;widget.dataset.open='false';widget.dataset.keyboard='false';widget.style.setProperty('--chat-keyboard-inset','0px');
...
byId('chat-expand').addEventListener('click',()=>{const expanded=panel.dataset.expanded!=='true';panel.dataset.expanded=String(expanded);byId('chat-expand').setAttribute('aria-pressed',String(expanded));byId('chat-expand').setAttribute('aria-label',expanded?'Restore chat size':'Expand chat');byId('chat-expand').textContent=expanded?'↙':'↗'});
